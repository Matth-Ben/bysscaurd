...
interface Props {
  ...
  onAddToast?: (message: string, type: 'info' | 'warning' | 'error' | 'success') => void;
  socket?: Socket | null;
  onResetUnreadCount?: (channelName: string) => void;
}

export default function Chat({ channel, channels, setChannels, fetchChannels, setSelectedChannel, session, onUnreadCountUpdate, onAddToast, socket, onResetUnreadCount }: Props) {
  ...
  useEffect(() => {
    ...
    if (channel) {
      // Demander l'historique du salon actuel
      ...
      socket.on("message", (msg: Message) => {
        ...
        console.log("Salon actuel:", channel);
        console.log("Message appartient au salon actuel:", msg.channel === channel);
        console.log("Messages actuels:", messages.length);
        
        // Afficher le message seulement s'il appartient au salon actuel
        if (msg.channel === channel) {
          ...
          console.log("❌ Message ignoré - appartient à un autre salon");
        }
        console.log("=== FIN DEBUG CHAT MESSAGES ===");
      });
  ...
  // Réinitialiser le compteur de messages non lus quand on entre dans un salon
  useEffect(() => {
    if (channel && onResetUnreadCount) {
      console.log("Réinitialisation du compteur pour le salon:", channel);
      onResetUnreadCount(channel);
      
      // Marquer tous les messages actuels comme lus
  ...
  }, [channel, onResetUnreadCount, messages]);
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => {
                ...
                onAddToast?.("Test de notification", 'info');
              }}
            ...
            <button
              onClick={() => {
                console.log("Test notification autre salon");
                onAddToast?.("Test: Message dans #AutreSalon", 'info');
              }}
              className="text-gray-400 hover:text-white transition-colors p-2 rounded hover:bg-[#40444b]"
              title="Tester notification autre salon"
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
